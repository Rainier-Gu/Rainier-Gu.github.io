  ...
  if (isLoading) {
    return (
      <div className="h-full w-full rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-6 flex flex-col items-center justify-center transition-colors duration-700">
        ...
        <span className="type-kicker text-slate-800 dark:text-white animate-pulse text-sm">CONNECTING...</span>
      </div>
    );
  }
  ...
  if (playlist.length === 0 || !currentSong) {
    return (
      <div className="h-full w-full rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-6 flex flex-col items-center justify-center transition-all duration-700">
        ...
        <span className="type-kicker text-slate-500 dark:text-slate-400 text-xs">No Music Available</span>
        <span className="type-summary text-[10px] text-slate-400 mt-1">请检查播放列表或网络连接</span>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-5 relative z-10 mb-6 mt-2">
          ...
          <div className="flex-col overflow-hidden w-full">
            <div className="flex items-center justify-between mb-1">
              <span className="type-kicker text-[12px] text-indigo-500 dark:text-indigo-400 bg-white/50 dark:bg-slate-900/50 px-2 py-0.5 rounded-sm shadow-sm transition-colors duration-700">{sourceLabel}</span>
            </div>
            <h3 className="type-card-title text-xl font-bold text-slate-900 dark:text-white truncate drop-shadow-sm transition-colors duration-700">{currentSong.title}</h3>
            <p className="type-meta text-sm text-slate-700 dark:text-slate-300 font-medium truncate drop-shadow-sm transition-colors duration-700">{currentSong.artist}</p>
          </div>
        </div>

        <div className="relative z-10 mb-2 h-6 overflow-hidden">
           <p className="type-summary text-xs font-bold text-indigo-600 dark:text-indigo-400 truncate">{displayedLyric}</p>
        </div>
